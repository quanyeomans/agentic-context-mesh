...
#     * Ensure the code is self-contained and includes necessary tags (`<!DOCTYPE html>`, `<html>`, `<head>`, ``, `<script>`, `<style>`).
...
#     * No `alert()`: Display messages (e.g., game over, score updates) using in-page HTML elements (e.g., ``, `<p>`) instead of the JavaScript `alert()` function.
