...
flowchart TD
    ...
    B{Does the use case fit endpoints?(i.e. B2B2C user-facing analytics, data-powered APIs, internal home-grown dashboards)}
    C["Explain the use case in #team-data-modeling (we're keen to talk to beta users!)"]
    D{Should they use batch exports instead?}
    E[Redirect them to start paying for batch exports.]
    F[1. Assume we're not increasing rate limits.2. Reach out to #team-clickhouse with query details.]
    G["Go to the relevant team for that API(Feature flags, Surveys, ..)"]
