...
- **Entity-modelling iter_5 enrichment deployment** (GH #343) — operator-driven cypher-shell load lifting production Neo4j from 871 nodes / 2 relationship types to ~27,000 nodes / 12 relationship types. Adds 5 new node labels (`Place`, `Product`, `Vocation`, `Industry`, `LegalCompliance`) and 10 new edge kinds (`LOCATED_IN_COUNTRY`, `HEADQUARTERED_IN`, `CITIZEN_OF`, `OPERATES_IN`, `HAS_OCCUPATION`, `DEVELOPED_BY`, `FIELD_OF`, `RUNS_ON`, `APPLIES_IN`, `CO_OCCURS_IN_CORPUS`). Curated values (`tier`, `engagement_status`, `vault_path`) preserved on existing hand-curated nodes; pipeline enrichment (`wikidata_qid`, `description`, `aliases`, demoted external-ref properties) augmented non-destructively. Every write tagged with `kairix_provenance_batch = 'iter5-2026-05-29'` for surgical rollback per the deployment plan. Search and entity-aware retrieval now operate against a fully enriched graph instead of the previous mostly-empty shape.
