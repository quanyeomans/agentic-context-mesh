...
### UX phase — dogfood-reported quality fixes

- **`kairix search` reads cleaner.** Long archive paths like `something.zip#1536` now show as `something.zip` in the title line (the full path stays on the path line below). Operators tune the snippet length with `--snippet-width N` — `200` for tighter triage output, `1200` for deep-dive readability. Closes #385.
- **`prep` no longer contradicts itself.** Previously the envelope could carry `summary: "No relevant content found..."` alongside a populated `sources: [...]` list — operators saw "synthesis says nothing matches yet retrieval listed N sources" and didn't know which half to trust. The envelope now drops the sources when the canned no-relevance summary fires so the two halves agree. Closes #433.
- **`contradict` stops flagging missing support as contradiction.** All three category prompts (DIRECT, OVERSTATEMENT, STATUS_MISMATCH) now explicitly score 0.0 when the snippet is silent on the claim. Previously chunks that didn't fully address every specific in a claim were flagged as if they actively contradicted it. Closes #434.
- **MCP cold-state regression self-heals.** If the in-process warm flag ever diverges from the persisted flag (the 13-hour cold-state regression class reported 2026-06-07), the MCP gate now detects the divergence at the request boundary, logs a WARN with the `warm_status` snapshot, re-marks in-process warm, and serves the request normally. Operators see a timestamped log of when the divergence fired instead of discovering it hours later via dogfood reports. Closes #425.

### Operator surface — YAML knobs now wire through end-to-end

- **Retrieval-quality knobs land in `kairix.config.yaml`.** Operators flip the new EPIC #438 controls via overlay — `retrieval.boosts.source_tier.enabled` (#432), `retrieval.boosts.content_quality.enabled` (#458), `retrieval.fact_layer_min_floor` / `retrieval.chunk_layer_min_floor` / `retrieval.cross_layer_dedup_enabled` (#455). All default to no-op, so existing deployments see byte-for-byte pre-feature ranking until you opt in.
- **Source-tier ranking gets two precision controls.** Declare a `canonical_filename_allowlist:` (e.g. `ETHOS.md`, `SOUL.md`, `AGENTS.md`) to lift specific files to the canonical tier regardless of their collection. Declare `per_intent_overrides:` to push certain tiers higher for specific query intents (e.g. canonical x5.0 for entity lookups). Both default empty.
