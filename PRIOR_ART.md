...
The combination of **Obsidian + QMD** as a retrieval stack was first observed gaining traction in the OpenClaw community ([Reddit/ObsidianMD, 2025](https://www.reddit.com/r/ObsidianMD/comments/1rix34w/)) and validated as a pattern worth building on.
...
**What influenced us:**  
The **single approved interface** architecture — rather than each agent maintaining its own retrieval stack, a shared search layer is queried by all agents. This is the direction Mnemosyne is moving: a shared retrieval service that all OpenClaw agents call, rather than per-agent vector stores.
...
## The TC Platform — Prior Internal Work

**Authors:** Triad Consulting (internal)  
**Status:** Superseded by OpenClaw stack (2026-03-07)  
...
**What we preserved:**  
The **Trusted Advisor research pipeline** — a 4-stage sequential pipeline (Understanding → Opportunity → Solution → Synthesis) for executive briefing preparation. Highly engineered prompts with evidence-grading, persona-specific framing, and dual-audience output (stakeholder-facing + advisor-facing). This pipeline's prompt templates and task descriptions are preserved in the vault under `04-Agent-Knowledge/shared/tc-platform-analysis.md` and will be ported to OpenClaw as a skill.

The **parallel specialist pattern** — 4 specialist agents (Chief of Staff, Industry Expert, Value Architect, Enterprise Architect) analyse the same research simultaneously, then a Director synthesises. A/B tested against sequential pipeline with equivalent quality. The separation-of-concerns framing produces better-evidenced outputs.

**Lesson:** Domain-specific intelligence (how to research a person, how to frame an opportunity, how to structure advice) travels across architectures. The infrastructure changed; the prompts remain valuable.
...
**What influenced us:**  
The **MCP server capability** — n8n workflows callable as tools from other AI systems. This bidirectional pattern (agents call workflows; workflows call agents) is how we think about the OpenClaw ↔ automation layer integration.
...
| TC Platform (internal) | Trusted Advisor prompt engineering | Vault `tc-platform-analysis.md`, pending skill port |
