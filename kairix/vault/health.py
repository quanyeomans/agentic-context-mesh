"""
kairix.vault.health — Vault and entity graph health check (Neo4j-primary).
...
Never raises — returns a VaultHealthReport on any failure.
"""
...
class VaultHealthReport:
    """
    Result of a vault + graph health check.
...
def run_vault_health(
    ...
    """
    Run vault + entity graph health check.
    ...
    Returns:
        VaultHealthReport describing entity graph state.
    """
    generated_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    report = VaultHealthReport(generated_at=generated_at)
    ...
    try:
        ...
        logger.warning("vault health: node count query failed — %s", exc)
    ...
    try:
        ...
        logger.warning("vault health: org vault_path check failed — %s", exc)
    ...
    try:
        ...
        logger.warning("vault health: person vault_path check failed — %s", exc)
    ...
    try:
        ...
        logger.warning("vault health: org summary check failed — %s", exc)
    ...
    try:
        ...
        logger.warning("vault health: person summary check failed — %s", exc)
    ...
    for rel_type, attr in [
        ...
        try:
            ...
            logger.warning("vault health: %s edge count failed — %s", rel_type, exc)

    # ── Surface issues ────────────────────────────────────────────────────────
    if report.total_entities == 0:
        report.issues.append("No entity nodes found in Neo4j — run `kairix vault crawl` first")
...
def format_health_text(report: VaultHealthReport) -> str:
    """Format a VaultHealthReport as human-readable text."""
    lines = [
        "# Kairix — Vault Health Report",
        f"_Generated: {report.generated_at}_",
        "",
    ]

    if not report.neo4j_available:
        lines += ["⚠ Neo4j unavailable — run `kairix vault crawl` to populate the graph", ""]
