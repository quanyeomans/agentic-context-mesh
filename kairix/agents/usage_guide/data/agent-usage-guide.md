...
<!--
  This file is the TEMPLATE for the bundled agent usage guide.
  The capability index between the GENERATED markers below is rendered from
  kairix.agents.mcp.server.CAPABILITIES_CATALOG — do NOT hand-edit it.
  Edit the prose here, then regenerate the shipped guide:
      python -m kairix.agents.usage_guide.generate
  The generated result is data/agent-usage-guide.md (the file agents read).
-->
...
- `remember` (MCP tool: `memory_write`) stores a single fact or decision now, so it can be recalled later.
- `ingest-chat` (MCP tool: `ingest_chat`) saves a whole chat transcript into the knowledge store for later recall.

You do **not** need to run diagnostics before every write — just write. Only *if* a write fails, run `kairix doctor agent --name <you>` (MCP tool: `doctor_check_agent`) to see what is misconfigured, fix it, and retry.
...
## How to name a tool on the wire

The **MCP tool** column in the capability index below is the exact name you put on the wire — it is **bare**: `search`, `expand`, `memory_write`. There is no `tool_` prefix and no `mcp-kairix__` prefix; those are documentation shorthands from older guides, not real tool names. If your client namespaces tools by server, it adds its own prefix — but the kairix-side name you pass is always the bare one shown here.
...
**MCP equivalents:** the same values (as strings) are accepted on the `scope` parameter of the `search`, `prep`, `timeline`, and `contradict` MCP tools.
...
### Research (MCP tool: research)
The `research` MCP tool runs iterative multi-turn search, refining queries until it finds a good answer. It always returns a synthesis — if no relevant documents are found, it synthesises what it can from the best available results rather than returning a failure message.
...
| Entity lookup returns nothing | Entity not in Neo4j | Run `kairix store crawl` |
...
## Capabilities — the loop-ordered index

This is the index for agents. Every kairix capability has one Python implementation with one or more bindings (CLI, MCP). The rows below are grouped by the working loop — **Orient → Find → Synthesise → Remember → Check health → Escalate** — so you can scan to the phase you're in rather than hunt an alphabetical list.

Each row gives you: when to reach for it, the **CLI** invocation, the **bare MCP tool name** you put on the wire, and whether it returns a `source_uri` breadcrumb you can cite or feed to `expand`. This table lists **every** kairix capability, so nothing you can call is left undocumented. If a capability is not here, it is not a live agent capability.

<!-- BEGIN GENERATED capability index (see kairix.agents.usage_guide.generate) -->

### Orient

Start here every session — learn the surface and warm the caches before you retrieve.

| capability | when to reach | CLI | MCP tool | returns `source_uri` |
|---|---|---|---|---|
| `usage_guide` | see the group note above | `kairix usage-guide` | `usage_guide` | — |
| `capabilities` | see the group note above | `kairix capabilities` | `capabilities` | — |
| `bootstrap` | see the group note above | `kairix bootstrap` | `bootstrap` | — |
| `entity_suggest` | see the group note above | `kairix entity suggest` | `entity_suggest` | — |
| `entity_validate` | see the group note above | `kairix entity validate` | `entity_validate` | — |

### Find

The read loop: `search` first, then `expand` / `timeline` / `entity` / `facts_about` the hits.

| capability | when to reach | CLI | MCP tool | returns `source_uri` |
|---|---|---|---|---|
| `search` | Call before answering any factual question about prior work or decisions. | `kairix search` | `search` | yes |
| `entity` | Look up a named person, organisation, or project across the knowledge store. | `kairix entity` | `entity` | yes |
| `timeline` | Trace how a topic or project changed over time, in date order. | `kairix timeline` | `timeline` | yes |
| `expand` | Pull the chunks surrounding a search hit instead of re-reading the whole document. | `kairix expand` | `expand` | yes |
| `facts_about` | Recall the stored facts about a person, project, or topic. | `kairix facts about` | `facts_about` | yes |

### Synthesise

When one hit isn't the whole answer — combine many, and cite the `source_uri` on each.

| capability | when to reach | CLI | MCP tool | returns `source_uri` |
|---|---|---|---|---|
| `prep` | Pull a tiered context summary before a meeting or a task hand-off. | `kairix prep` | `prep` | yes |
| `research` | Gather and synthesise everything the store knows about a broad question. | `kairix research` | `research` | yes |
| `contradict` | Check new content for conflicts with what the store already knows. | `kairix contradict` | `contradict` | yes |
| `brief` | Produce a session briefing that synthesises recent activity. | `kairix brief` | `brief` | yes |

### Remember

The write loop — save a fact or transcript now so the next session can recall it.

| capability | when to reach | CLI | MCP tool | returns `source_uri` |
|---|---|---|---|---|
| `ingest_chat` | Save a chat transcript into the knowledge store for later recall. | `kairix ingest-chat` | `ingest_chat` | — |
| `memory_write` | Remember a fact or decision now so it can be recalled later. | `kairix remember` | `memory_write` | — |

### Check health

Agent-callable diagnostics — run one when a call misbehaves, before giving up.

| capability | when to reach | CLI | MCP tool | returns `source_uri` |
|---|---|---|---|---|
| `onboard_check` | see the group note above | `kairix onboard check` | `onboard_check` | — |
| `onboard_scan` | see the group note above | `kairix onboard scan` | `onboard_scan` | — |
| `onboard_agent` | see the group note above | `kairix onboard agent` | `onboard_agent` | — |
| `doctor_check_all` | see the group note above | `kairix doctor agent --all` | `doctor_check_all` | — |
| `doctor_check_agent` | see the group note above | `kairix doctor agent --name` | `doctor_check_agent` | — |
| `worker_status` | see the group note above | `kairix worker status` | `worker_status` | — |
| `features_status` | see the group note above | `kairix features status` | `features_status` | — |
| `secrets_verify` | see the group note above | `kairix secrets verify` | `secrets_verify` | — |
| `dead_letter_status` | see the group note above | `kairix dead-letter status` | `dead_letter_status` | — |
| `caches_status` | see the group note above | `kairix caches` | `caches_status` | — |
| `warm` | see the group note above | `kairix warm` | `warm` | — |
| `probe_search` | see the group note above | `python -c 'from kairix.quality.probe import run_probe_search; ...'` | `probe_search` | — |
| `maintenance_analyze` | see the group note above | `kairix maintenance analyze` | `maintenance_analyze` | — |

### Escalate

Operator-only — the MCP tool returns an escalation envelope you hand to your human.

| capability | when to reach | CLI | MCP tool | returns `source_uri` |
|---|---|---|---|---|
| `soak_run` | see the group note above | `python -c 'from kairix.quality.soak import run_soak; ...'` | operator only (escalate: `soak_run`) | — |
| `benchmark_run` | see the group note above | `kairix benchmark run` | operator only (escalate: `benchmark_run`) | — |
| `probe_burst` | see the group note above | `python -c 'from kairix.quality.probe import run_probe_burst; ...'` | operator only (escalate: `probe_burst`) | — |
| `probe_config` | see the group note above | `kairix probe-config` | operator only (escalate: `probe_config`) | — |
| `embed` | see the group note above | `kairix embed` | operator only (escalate: `embed`) | — |
| `store_crawl` | see the group note above | `kairix store crawl` | operator only (escalate: `store_crawl`) | — |
| `embed_rebuild_fts` | see the group note above | `kairix embed rebuild-fts` | operator only (escalate: `embed_rebuild_fts`) | — |
| `cc_pair` | see the group note above | `kairix cc-pair` | operator only (escalate: `cc_pair`) | — |

<!-- END GENERATED capability index -->

The **Escalate** rows return an `OperatorOnlyCapability` envelope over MCP — surface the `operator_command` field to your admin when you need that work done. Everything in the other five groups you can call yourself.
