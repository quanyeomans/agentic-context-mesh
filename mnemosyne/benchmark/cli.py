"""
CLI entry point for `mnemosyne benchmark`.

Usage:
  mnemosyne benchmark run     --suite SUITE [--system hybrid|bm25] [--agent AGENT] [--output DIR]
  mnemosyne benchmark validate --suite SUITE
  mnemosyne benchmark compare  RESULT_A RESULT_B
  mnemosyne benchmark init    --agent AGENT [--collections COL,COL]
...
def cmd_run(args: argparse.Namespace) -> int:
    from mnemosyne.benchmark.runner import format_interpretation, run_benchmark
    from mnemosyne.benchmark.suite import load_suite, validate_suite
...
def cmd_compare(args: argparse.Namespace) -> int:
    ...
    from mnemosyne.benchmark.runner import CATEGORY_WEIGHTS, _score_tier
...
# Generated by `mnemosyne benchmark init --agent {agent}`
...
"""
    ...
    print(f"   Edit the file and run: mnemosyne benchmark validate --suite {output}")
    return 0
