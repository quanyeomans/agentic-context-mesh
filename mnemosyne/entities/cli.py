...
Usage:
  mnemosyne entity write --name "Alex Jordan" --type person [--summary "..."] [--fact "CEO, 2024"]
  mnemosyne entity lookup "Alex Jordan"
  mnemosyne entity list [--type person]
  mnemosyne entity extract --changed
  mnemosyne entity extract --all
  mnemosyne entity extract --path file.md
  mnemosyne entity extract --use-llm
...
def _write_cmd(args: argparse.Namespace) -> None:
    from mnemosyne.entities.graph import entity_write, set_vault_path, slug
...
def _list_cmd(args: argparse.Namespace) -> None:
    from mnemosyne.entities.graph import entity_list
    from mnemosyne.entities.schema import open_entities_db
...
def _extract_cmd(args: argparse.Namespace) -> None:
    ...
    from mnemosyne.entities.pipeline import (
        VAULT_ROOT,
        get_changed_files,
        get_last_run_mtime,
...
def _reconcile_cmd(args: argparse.Namespace) -> None:
    ...
    from mnemosyne.entities.reconcile import _string_similarity
    from mnemosyne.entities.schema import open_entities_db
    ...
    if args.report:
        print("\nInfo: Report only -- no changes written. Use `mnemosyne entity write` to manually merge.")


def main(argv: list[str] | None = None) -> None:
    """Entry point for `mnemosyne entity` subcommand."""
    parser = argparse.ArgumentParser(
        prog="mnemosyne entity",
    ...
    write_parser.add_argument("--name", required=True, help="Entity name (e.g. 'Alex Jordan')")
    ...
    write_parser.add_argument(
        ...
        default=None,
        dest="vault_path",
        help="Vault folder/file path for wikilink ontology (e.g. '02-Areas/Work/Clients/AcmeHealth/')",
    )
