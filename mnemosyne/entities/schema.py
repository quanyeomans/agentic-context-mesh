...
Environment variable:
  MNEMOSYNE_TEST_DB — override DB path for testing (avoids touching /data/mnemosyne/)
"""
...
def open_entities_db(path: str | None = None) -> sqlite3.Connection:
    ...
    If MNEMOSYNE_TEST_DB env var is set, that path overrides both `path` and DEFAULT_DB_PATH.
    Configures WAL journal mode and calls ensure_schema() before returning.

    Args:
        path: Optional explicit path. Ignored if MNEMOSYNE_TEST_DB is set.

    Returns:
        An open sqlite3.Connection with schema applied.
    """
    test_db = os.environ.get("MNEMOSYNE_TEST_DB")
    if test_db:
        db_path = test_db
...
def ensure_schema(db: sqlite3.Connection) -> None:
    ...
    if current_version > SCHEMA_VERSION:
        raise SchemaVersionError(
            ...
            "Update SCHEMA_VERSION in mnemosyne/entities/schema.py after reviewing changes."
        )
