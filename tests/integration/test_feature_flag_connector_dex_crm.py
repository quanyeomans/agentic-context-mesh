"""Integration tests for the ``connector_dex_crm`` flag.
...
NOT a dead dispatcher's branch-marker log.
  ...
  * **Flag OFF** — ``connector_dex_crm`` reads False. The Dex CRM entry is gated
    ...
    sibling.
  * **Flag ON** — ``connector_dex_crm`` reads True. The gate lets the Dex CRM
    ...
    loop continues). The "gated off" log does NOT fire for dex_crm; the
...
"gated off" log no longer fires and the dex_crm entry runs. Restoring the
...
def test_flag_off_gates_connector_sibling_still_runs(
    ...
    """OFF — the dex_crm entry is gated off in the loop; the flagless obsidian
    sibling still syncs both notes.

    With the flag OFF the predicate skips the dex_crm entry BEFORE plugin
...
def test_flag_on_lets_connector_through_sibling_still_runs(
    ...
    """ON — the gate lets the dex_crm entry through to the batch runner; the
    flagless obsidian sibling still syncs both notes.

    With the flag ON the predicate does NOT skip the dex_crm entry: it
    ...
    continues). The "gated off" log must NOT fire for dex_crm — proving the
