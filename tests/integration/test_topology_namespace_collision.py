"""Integration coverage for the #305 ``topology:`` parent-key namespace.
...
which forced ``kairix.core.search.config_validator._validate_topology``
...
#305 nests the six blocks under a single ``topology:`` parent key.
...
1. The validator now wires rule 3 end-to-end — a malformed YAML where a
   ``topology.collections.*.sources.*.cc_pair`` references a
...
2. The validator now wires every other cross-reference rule
   (1, 2, 4, 5) end-to-end via the ``topology:`` parent.
3. A legacy config with the top-level ``collections.shared`` dict shape
   AND no ``topology:`` block still parses + validates exactly the
...
def test_rule3_fires_for_dangling_collection_source_cc_pair() -> None:
    """A ``topology.collections.*.sources.*.cc_pair`` reference that does not
    match any declared ``topology.cc_pairs.*.id`` surfaces rule 3.
    ...
    ``_validate_topology`` to skip ``data.get("topology")`` again
...
def test_validator_wires_remaining_rules_through_topology_parent() -> None:
    """The other cross-reference rules (1 / 2 / 4 / 5) also fire end-to-end
    through the namespaced ``topology:`` parent — the validator no
...
def test_legacy_collections_shared_dict_shape_still_validates() -> None:
    ...
    ``collections.shared`` dict shape AND no ``topology:`` parent key
    ...
    additive at the top level: it adds ``topology:`` without removing
    ...
    ``_validate_topology`` to always raise turns this test red
    because the top-level legacy ``collections.shared`` path still runs
    the topology validator (with an absent parent key → no-op). The
...
def test_legacy_collections_shared_with_duplicate_name_still_reports_error() -> None:
    ...
    independent of the topology surface) keeps surfacing its
    ...
    legacy validator while moving the topology surface.
    """
    legacy_yaml_dict = {
        "collections": {
            "shared": [
...
def test_legacy_config_with_topology_block_runs_both_validators() -> None:
    """A hybrid config that carries BOTH the legacy ``collections.shared``
    dict AND a ``topology:`` parent (operator partway through the
