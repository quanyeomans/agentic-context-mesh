"""Integration tests for the ``connector_linear`` flag.
...
NOT a dead dispatcher's branch-marker log.
  ...
  * **Flag OFF** — ``connector_linear`` reads False. The Linear entry is gated
    ...
    sibling.
  * **Flag ON** — ``connector_linear`` reads True. The gate lets the Linear
    ...
    loop continues). The "gated off" log does NOT fire for linear; the
...
"gated off" log no longer fires and the linear entry runs. Restoring the
gate returns the test to green.

Spec: docs/architecture/connector-scope-topology/connector-design-specs/linear.md
"""
...
def test_flag_off_gates_connector_sibling_still_runs(
    ...
    """OFF — the linear entry is gated off in the loop; the flagless obsidian
    sibling still syncs both notes.

    With the flag OFF the predicate skips the linear entry BEFORE plugin
...
def test_flag_on_lets_connector_through_sibling_still_runs(
    ...
    """ON — the gate lets the linear entry through to the batch runner; the
    flagless obsidian sibling still syncs both notes.

    With the flag ON the predicate does NOT skip the linear entry: it
    ...
    continues). The "gated off" log must NOT fire for linear — proving the
