...
@given(parsers.parse("the operator has the dex crm connector flag set to {value}"))
def _operator_sets_flag(dex_crm_flag_ctx: _Ctx, value: str) -> None:
    """Pin the flag's value via :class:`FakeFeatureFlagResolver`."""
    parsed = value.strip().lower() == "true"
    dex_crm_flag_ctx.resolver = FakeFeatureFlagResolver().with_flag(_FLAG, parsed)


@given("a flagless sibling connector with two notes is configured alongside dex crm")
def _sibling_configured(dex_crm_flag_ctx: _Ctx, tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    (vault / "alpha.md").write_text("# Alpha\n\nFirst note body.\n", encoding="utf-8")
    (vault / "beta.md").write_text("# Beta\n\nSecond note body.\n", encoding="utf-8")
    dex_crm_flag_ctx.sibling_vault = vault


@when("the worker connector sync tick runs for dex crm")
def _worker_tick_runs(
    dex_crm_flag_ctx: _Ctx,
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Invoke the production :func:`run_connector_sync_pipeline`."""
    resolver = dex_crm_flag_ctx.resolver
    vault = dex_crm_flag_ctx.sibling_vault
    ...
    with caplog.at_level(logging.INFO, logger="kairix.worker"):
        dex_crm_flag_ctx.result = run_connector_sync_pipeline(deps)

    dex_crm_flag_ctx.captured_logs = [rec.getMessage() for rec in caplog.records]
...
def _gated_off(dex_crm_flag_ctx: _Ctx) -> None:
    assert _gated_logs(dex_crm_flag_ctx), (
        f"expected a '{_KIND} gated off' INFO log; got {dex_crm_flag_ctx.captured_logs!r}"
    )


@then("the dex crm connector is not gated off in the loop")
def _not_gated_off(dex_crm_flag_ctx: _Ctx) -> None:
    assert not _gated_logs(dex_crm_flag_ctx), (
        f"flag ON must NOT gate off {_KIND}; gated logs fired anyway: {_gated_logs(dex_crm_flag_ctx)!r}"
    )


@then("the flagless sibling connector still syncs its notes for dex crm")
def _sibling_syncs(dex_crm_flag_ctx: _Ctx) -> None:
    result = dex_crm_flag_ctx.result
