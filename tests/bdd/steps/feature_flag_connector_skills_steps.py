...
def _operator_sets_flag(skills_flag_ctx: _Ctx, value: str) -> None:
    ...
    skills_flag_ctx.resolver = FakeFeatureFlagResolver().with_flag(_FLAG, parsed)


@given("a flagless sibling connector with two notes is configured alongside skills")
def _sibling_configured(skills_flag_ctx: _Ctx, tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    (vault / "alpha.md").write_text("# Alpha\n\nFirst note body.\n", encoding="utf-8")
    (vault / "beta.md").write_text("# Beta\n\nSecond note body.\n", encoding="utf-8")
    skills_flag_ctx.sibling_vault = vault


@when("the worker connector sync tick runs for skills")
def _worker_tick_runs(
    skills_flag_ctx: _Ctx,
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Invoke the production :func:`run_connector_sync_pipeline`."""
    resolver = skills_flag_ctx.resolver
    vault = skills_flag_ctx.sibling_vault
    ...
    with caplog.at_level(logging.INFO, logger="kairix.worker"):
        skills_flag_ctx.result = run_connector_sync_pipeline(deps)
...
def _gated_off(skills_flag_ctx: _Ctx) -> None:
    assert _gated_logs(skills_flag_ctx), (
        f"expected a '{_KIND} gated off' INFO log; got {skills_flag_ctx.captured_logs!r}"
    )


@then("the skills connector is not gated off in the loop")
def _not_gated_off(skills_flag_ctx: _Ctx) -> None:
    assert not _gated_logs(skills_flag_ctx), (
        f"flag ON must NOT gate off {_KIND}; gated logs fired anyway: {_gated_logs(skills_flag_ctx)!r}"
    )


@then("the flagless sibling connector still syncs its notes for skills")
def _sibling_syncs(skills_flag_ctx: _Ctx) -> None:
    result = skills_flag_ctx.result
