...
def _operator_sets_flag(gmail_flag_ctx: _Ctx, value: str) -> None:
    """Pin the flag's value via :class:`FakeFeatureFlagResolver`."""
    parsed = value.strip().lower() == "true"
    gmail_flag_ctx.resolver = FakeFeatureFlagResolver().with_flag(_FLAG, parsed)


@given("a flagless sibling connector with two notes is configured alongside gmail")
def _sibling_configured(gmail_flag_ctx: _Ctx, tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    vault.mkdir()
    (vault / "alpha.md").write_text("# Alpha\n\nFirst note body.\n", encoding="utf-8")
    (vault / "beta.md").write_text("# Beta\n\nSecond note body.\n", encoding="utf-8")
    gmail_flag_ctx.sibling_vault = vault
...
def _worker_tick_runs(
    ...
    resolver = gmail_flag_ctx.resolver
    vault = gmail_flag_ctx.sibling_vault
    ...
    with caplog.at_level(logging.INFO, logger="kairix.worker"):
        gmail_flag_ctx.result = run_connector_sync_pipeline(deps)

    gmail_flag_ctx.captured_logs = [rec.getMessage() for rec in caplog.records]
...
def _gated_off(gmail_flag_ctx: _Ctx) -> None:
    assert _gated_logs(gmail_flag_ctx), f"expected a '{_KIND} gated off' INFO log; got {gmail_flag_ctx.captured_logs!r}"


@then("the gmail connector is not gated off in the loop")
def _not_gated_off(gmail_flag_ctx: _Ctx) -> None:
    assert not _gated_logs(gmail_flag_ctx), (
        f"flag ON must NOT gate off {_KIND}; gated logs fired anyway: {_gated_logs(gmail_flag_ctx)!r}"
    )


@then("the flagless sibling connector still syncs its notes for gmail")
def _sibling_syncs(gmail_flag_ctx: _Ctx) -> None:
    result = gmail_flag_ctx.result
