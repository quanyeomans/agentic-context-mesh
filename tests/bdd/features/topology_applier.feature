...
Feature: Wave D apply-bridge materialises parsed config into runtime topology rows
  As an operator declaring topology.cc_pairs and topology.collections
  ...
  Scenario: applying a complete topology declares cc_pairs and collections
    Given the operator has declared a topology config with one connector, one credential, one cc_pair, and one collection
