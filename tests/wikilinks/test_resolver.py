...
|---|---|---|
| Acme Health | `[[AcmeHealth]]` | `02-Areas/Work/Clients/AcmeHealth/` |
| Zenith Energy | `[[ZenithEnergy]]` | `02-Areas/Work/Clients/ZenithEnergy/` |
| Burger Palace | `[[BurgerPalace\\|Burger Palace]]` | `02-Areas/Work/Clients/BurgerPalace/` |
| Bridgewater Engineering | `[[BridgewaterEngineering]]` | `02-Areas/Work/Clients/BridgewaterEngineering/` |
...
| Entity | Link | Vault Path |
|---|---|---|
| Alex Jordan | `[[AlexJordan]]` | `02-Areas/People/AlexJordan/` |
...
def synthetic_db(tmp_path: Path) -> str:
    ...
    rows = [
        (
            ...
            "2024-01-01",
            "2024-01-01",
            "02-Areas/Work/Clients/AcmeHealth/",
            None,
        ),
        (
            "zenith-energy",
            ...
            "2024-01-01",
            "2024-01-01",
            "02-Areas/Work/Clients/ZenithEnergy/",
            None,
        ),
        (
            "nexus-digital",
...
def test_bootstrap_parses_acme_health(bootstrap_file: str) -> None:
    """Acme Health should be present with correct link and vault_path."""
    entities = load_entities_from_bootstrap(bootstrap_file)
    acme = next((e for e in entities if e.name == "Acme Health"), None)
    assert acme is not None, "Acme Health not found in bootstrap entities"
    assert acme.link == "[[AcmeHealth]]"
    assert "AcmeHealth" in acme.vault_path


def test_bootstrap_parses_burger_palace_alias(bootstrap_file: str) -> None:
    """Burger Palace should parse the display-alias link form correctly."""
    entities = load_entities_from_bootstrap(bootstrap_file)
    bp = next((e for e in entities if "Burger" in e.name), None)
    assert bp is not None, "Burger Palace not found in bootstrap entities"
    assert bp.link == "[[BurgerPalace|Burger Palace]]"
...
def test_db_load_skips_entities_without_vault_path(tmp_path: Path) -> None:
    ...
    db.execute(
        "INSERT INTO entities VALUES "
        "('e1', 'Acme Health', 'client', 'path', 'active', 'shared', "
        "'2024', '2024', '02-Areas/Work/Clients/AcmeHealth/')"
    )
    db.execute(
...
def test_db_load_returns_empty_without_vault_path_column(tmp_path: Path) -> None:
    ...
    db.execute("INSERT INTO entities VALUES ('e1', 'Acme Health', 'client', 'path')")
    db.commit()
    db.close()
...
def test_get_entities_falls_back_to_bootstrap_when_db_sparse(
    ...
    db.execute(
        "INSERT INTO entities VALUES "
        "('e2', 'Zenith Energy', 'client', 'path', 'active', 'shared', "
        "'2024', '2024', '02-Areas/Work/Clients/ZenithEnergy/')"
    )
    db.commit()
    db.close()
...
def test_wiki_entity_all_triggers_deduplicates() -> None:
    ...
    entity = WikiEntity(
        name="Acme Health",
        aliases=["Acme Health", "Acme", "AH"],
        vault_path="02-Areas/Work/Clients/AcmeHealth/",
        link="[[AcmeHealth]]",
        entity_type="client",
    )
    triggers = entity.all_triggers()
    assert len(triggers) == len(set(triggers)), "Duplicate triggers found"
    assert "Acme Health" in triggers
