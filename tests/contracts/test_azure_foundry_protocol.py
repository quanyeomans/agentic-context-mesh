"""Contract test for the Azure AI Foundry provider plugin (F43).

Exercises the canonical fake (:class:`tests.fakes.FakeProvider` configured
with ``name="azure_foundry"``) AND the real implementation
(:class:`kairix.providers.azure_foundry.AzureFoundryProvider`) through
the same :class:`~kairix.providers.Provider` Protocol assertions. F43
...
hand-rolled in-memory transport client that mirrors the Azure-OpenAI
SDK surface (``embeddings.create`` and ``chat.completions.create``) —
the contract assertions check shape (typed return values, ``name``
attribute, callable methods), not delivery latency.
"""
...
from kairix.providers.azure_foundry import PROVIDER_NAME, AzureFoundryProvider
...
class _StubFoundryClient:
    """Minimal transport stub satisfying what :class:`AzureFoundryProvider` consumes."""
...
def _real_factory() -> Provider:
    creds = Credentials(
        api_key="foundry-test-key",  # pragma: allowlist secret — test fixture
        endpoint="https://example.services.ai.azure.com/openai/v1",
        model="text-embedding-3-large",
        dims=3,
    )
    return AzureFoundryProvider(credentials=creds, transport_client=_StubFoundryClient())
...
def test_azure_foundry_provider_satisfies_provider_protocol(name: str, factory: Callable[[], Provider]) -> None:
    """F43: both fake and real impl satisfy the runtime-checkable Protocol.

    Sabotage-proof: deleting ``embed_batch`` from
    :class:`AzureFoundryProvider` flips the real-impl isinstance check
    to False; same for the fake.
    """
    provider = factory()
...
@pytest.mark.contract
@pytest.mark.parametrize("name,factory", _FACTORIES)
def test_azure_foundry_provider_exposes_protocol_callables(name: str, factory: Callable[[], Provider]) -> None:
...
@pytest.mark.contract
@pytest.mark.parametrize("name,factory", _FACTORIES)
def test_azure_foundry_provider_dimension_returns_positive_int(name: str, factory: Callable[[], Provider]) -> None:
...
@pytest.mark.contract
@pytest.mark.parametrize("name,factory", _FACTORIES)
def test_azure_foundry_provider_healthcheck_returns_provider_health(name: str, factory: Callable[[], Provider]) -> None:
