"""Contract test for the LiteLLM-proxy provider plugin (F43).

Exercises the canonical fake (:class:`tests.fakes.FakeProvider` configured
with ``name="litellm_proxy"``) AND the real implementation
(:class:`kairix.providers.litellm_proxy.LiteLLMProxyProvider`) through the
same :class:`~kairix.providers.Provider` Protocol assertions. F43 requires
this pairing — without it the canonical fake can drift away from the real
wire (or vice versa) and the production path silently diverges from what
...
surface the LiteLLM proxy presents — the contract assertions check shape
(typed return values, ``name`` attribute, callable methods), not delivery
latency.
"""
...
from kairix.providers.litellm_proxy import PROVIDER_NAME, LiteLLMProxyProvider
...
class _StubProxyClient:
    """Minimal transport stub satisfying what the proxy plugin consumes."""
...
def _real_factory() -> Provider:
    creds = Credentials(
        api_key="proxy-test-key",  # pragma: allowlist secret — test fixture
        endpoint="http://localhost:4000",
        model="text-embedding-3-large",
        dims=3,
    )
    return LiteLLMProxyProvider(credentials=creds, transport_client=_StubProxyClient())
...
def test_litellm_proxy_provider_satisfies_provider_protocol(name: str, factory: Callable[[], Provider]) -> None:
    """F43: both fake and real impl satisfy the runtime-checkable Protocol.

    Sabotage-proof: deleting ``embed_batch`` from
    :class:`LiteLLMProxyProvider` flips the real-impl isinstance check to
...
@pytest.mark.contract
@pytest.mark.parametrize("name,factory", _FACTORIES)
def test_litellm_proxy_provider_exposes_protocol_callables(name: str, factory: Callable[[], Provider]) -> None:
...
@pytest.mark.contract
@pytest.mark.parametrize("name,factory", _FACTORIES)
def test_litellm_proxy_provider_dimension_returns_positive_int(name: str, factory: Callable[[], Provider]) -> None:
...
@pytest.mark.contract
@pytest.mark.parametrize("name,factory", _FACTORIES)
def test_litellm_proxy_provider_healthcheck_returns_provider_health(name: str, factory: Callable[[], Provider]) -> None:
