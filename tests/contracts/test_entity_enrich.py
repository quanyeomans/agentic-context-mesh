...
class _RecordingNeo4j:
    ...
    Returns ``lookup_rows`` for the first call, empty list for subsequent
    calls — mirrors the read-then-write flow inside enrich_entity.
    """
...
def test_enrich_entity_writes_summary_via_set_cypher() -> None:
    ...
    write_calls = [(q, p) for q, p in neo4j.cypher_calls if "SET n.summary" in q]
...
def test_enrich_entity_is_idempotent_on_populated_summary() -> None:
    ...
    write_calls = [q for q, _ in neo4j.cypher_calls if "SET n.summary" in q]
