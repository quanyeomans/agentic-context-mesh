...
@pytest.mark.unit
def test_slug_alice_chen():
    assert slug("Alice Chen") == "alice-chen"


@pytest.mark.unit
def test_slug_acme_corp():
    assert slug("Acme Corp") == "acme-corp"


@pytest.mark.unit
def test_slug_special_chars():
    assert slug("platform/qmd") == "platform-qmd"
...
def test_entity_write_twice_does_not_duplicate(db, vault_root):
    ...
    count = db.execute("SELECT COUNT(*) FROM entities WHERE id = 'alice-chen'").fetchone()[0]
    assert count == 1

    row = db.execute("SELECT summary FROM entities WHERE id = 'alice-chen'").fetchone()
...
def test_entity_write_with_facts(db, vault_root):
    ...
    entity_write(
        ...
        markdown_path="06-Entities/person/alice-chen.md",
        db=db,
        facts=["CEO, 2024", "Founder of Acme Corp"],
    )
    rows = db.execute("SELECT fact_text FROM entity_facts WHERE entity_id = 'alice-chen' ORDER BY id").fetchall()
    fact_texts = [r[0] for r in rows]
    assert "CEO, 2024" in fact_texts
    assert "Founder of Acme Corp" in fact_texts
...
def test_entity_write_creates_vault_file(db, vault_root):
    ...
    vault_file = vault_root / "06-Entities/organisation/acme-corp.md"
    assert vault_file.exists()
    content = vault_file.read_text()
    assert "entity-id: acme-corp" in content
...
def test_entity_lookup_exact_match(db, vault_root):
    ...
    result = entity_lookup("Alice Chen", db)
...
def test_entity_lookup_returns_relationships(db, vault_root):
    ...
    db.execute(
        ...
        VALUES ('alice-chen', 'acme-corp', 'member_of', ?, ?)
        """,
        (now, now),
    )
    db.commit()
...
def test_get_mentions_ordering(db, vault_root):
    ...
    mentions = get_mentions("alice-chen", db, limit=10)
...
def test_get_mentions_limit(db, vault_root):
    ...
    for i in range(15):
        db.execute(
            """
            INSERT INTO entity_mentions (entity_id, doc_uri, mention_count, first_seen, last_seen)
            VALUES ('alice-chen', ?, ?, '2026-01-01', '2026-03-01')
            """,
            (f"vault://doc-{i}.md", i + 1),
        )
    db.commit()

    mentions = get_mentions("alice-chen", db, limit=5)
