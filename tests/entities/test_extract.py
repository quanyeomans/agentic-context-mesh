...
def vault_root(tmp_path):
    ...
    (vault / "02-Areas" / "Work" / "Clients" / "Acme Health").mkdir(parents=True)
    (vault / "02-Areas" / "Work" / "Clients" / "BurgerPalace").mkdir(parents=True)
    (vault / "01-Projects" / "Mnemosyne").mkdir(parents=True)
    # Also a file
    (vault / "02-Areas" / "Work" / "Clients" / "Acme Health" / "Overview.md").write_text(
        "# Acme Health\nHealth insurance client.", encoding="utf-8"
    )
    _invalidate_vault_cache()
    return vault
...
def db_with_entities(db):
    ...
    db.execute(
        ...
        (
            ...
            "organisation",
            "Triad Consulting",
            "06-Entities/organisation/triad-consulting.md",
            now,
            now,
        ),
    )
    db.execute(
        ...
        VALUES (?, ?, ?, 'active', ?, 'shared', ?, ?)
        """,
        ("alex-jordan", "person", "Alex Jordan", "06-Entities/person/alex-jordan.md", now, now),
    )
    db.commit()
    return db
...
def test_extract_detects_vault_folder_names(db, vault_root):
    ...
    _invalidate_vault_cache()
    text = "We recently partnered with Acme Health for the health insurance initiative."
    results = extract_rules_based_with_db(text, "/vault/notes.md", db, vault_root=vault_root)

    names = [e.name for e in results]
    assert "Acme Health" in names, f"Expected 'Acme Health' in extracted names, got: {names}"
...
def test_extract_multiple_vault_names(db, vault_root):
    ...
    _invalidate_vault_cache()
    text = "We discussed Acme Health and BurgerPalace during the strategy session."
    results = extract_rules_based_with_db(text, "/vault/notes.md", db, vault_root=vault_root)
    names = {e.name for e in results}
    assert "Acme Health" in names
    assert "BurgerPalace" in names
...
def test_extract_detects_db_entity_names(db_with_entities, vault_root):
    ...
    _invalidate_vault_cache()
    text = "Alex Jordan presented the Triad Consulting roadmap at the all-hands."
    results = extract_rules_based_with_db(text, "/vault/notes.md", db_with_entities, vault_root=vault_root)
    names = [e.name for e in results]
    assert "Alex Jordan" in names, f"Expected 'Alex Jordan', got: {names}"
    assert "Triad Consulting" in names, f"Expected 'Triad Consulting', got: {names}"
...
def test_extract_db_entity_confidence_is_high(db_with_entities, vault_root):
    ...
    _invalidate_vault_cache()
    text = "Alex Jordan is the CEO."
    results = extract_rules_based_with_db(text, "/vault/notes.md", db_with_entities, vault_root=vault_root)
    dan_results = [e for e in results if e.name == "Alex Jordan"]
    assert dan_results, "Alex Jordan should be extracted"
    assert dan_results[0].confidence >= 0.9
...
def test_extract_captures_title_case_mid_sentence(db, vault_root):
    ...
    text = "Alpha Beta is our internal tool. We use Alpha Beta daily at Triad Consulting."
    # "Alpha Beta" appears mid-sentence in second sentence
    results = extract_rules_based_with_db(text, "/vault/notes.md", db, vault_root=vault_root)
    names = [e.name for e in results]  # noqa: F841
    # Triad Consulting might match DB or heuristic; just check we got something mid-sentence
...
def test_extract_file_skips_llm_when_rules_sufficient(tmp_path, db_with_entities, vault_root, monkeypatch):
    ...
    md_file.write_text(
        "# Meeting Notes\n\nAlex Jordan met with the Triad Consulting team."
    ...
    assert len(results) >= 2  # at least Alex Jordan + Triad Consulting (or Mnemosyne)
...
def test_extract_file_strips_frontmatter(tmp_path, db, vault_root, monkeypatch):
    ...
    md_file.write_text(
        "---\ntitle: Test\ntype: note\n---\n\nWe partnered with Acme Health for health insurance.\n",
        encoding="utf-8",
    )

    results = extract_file(str(md_file), db, vault_root=vault_root)
    names = [e.name for e in results]
    # Acme Health is a vault folder name
    assert "Acme Health" in names
...
def test_read_stub_aliases_real_entity_stub():
    ...
    stub_path = "/data/obsidian-vault/04-Agent-Knowledge/entities/concept/builder.md"
