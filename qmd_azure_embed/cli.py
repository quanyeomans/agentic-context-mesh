...
LOCKFILE = Path("/tmp/qmd-embed.lock")
LOCK_WAIT_SECS = 60
...
def cmd_embed(args) -> int:
    ...
    logging.info(
        f"qmd-azure-embed starting — force={args.force} limit={args.limit} "
    ...
    try:
        ...
        if result["failed"] > 0:
            logging.warning(
                f"{result['failed']} chunks failed. "
                f"Re-run without --force to retry failed chunks."
            )
    ...
    logging.info(
        f"Recall: {recall_result['passed']}/{recall_result['total']} "
        f"({recall_result['score']:.0%})"
    )
...
def main():
    ...
    embed_p.add_argument("--limit", type=int, default=None,
                         help="Cap total chunks (for validation)")
    embed_p.add_argument("--batch-size", type=int, default=100,
                         help="Chunks per Azure API call")
    embed_p.add_argument("--skip-recall-check", action="store_true",
                         help="Skip post-embed quality gate")
