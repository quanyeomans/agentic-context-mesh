...
def check_recall(db: sqlite3.Connection | None = None) -> dict:
    ...
    for qid, query, gold_fragment in queries:
        ...
        if query_vec is None:
            detail.append({"id": qid, "query": query, "gold_fragment": gold_fragment,
                           "hit": False, "returned": [], "skipped": True})
            continue
...
def run_recall_gate(alert_callback=None) -> tuple[bool, dict]:
    """
