...
- **F73** no private infrastructure references in committed code, docs, or tests. Specific Azure resource names (VM, Key Vault, storage account, resource group, lock pattern), internal hostnames / email domains, and private sibling-repo identifiers are matched by a curated regex set in `scripts/checks/check_no_private_infra_refs.py`. Generic placeholders (`<your-vm-name>`, `<your-key-vault-name>`, `<your-resource-group>`, `example.com`) document the shape without naming the operator's deployment. Complements F32 (which targets real first / surname / org names in fixtures). Baseline at `.architecture/baseline/no-private-infra-refs-files.txt` ships empty — the W1+W3 redaction sweeps paid down every then-existing offender to zero. Motivation: kairix is a public repository; specific identifiers leak organisational shape and aid reconnaissance.
