...
- **F62** every stateful tick/run_batch component has a multi-tick advance/idempotency test. **F68** every Protocol method — discovered repo-wide across `kairix/**/*.py`, not just `protocols.py` — has a failure-injection contract test (one of `raises`/`times_out`/`returns_partial`/`returns_empty`/`unauthorized`/`unavailable`). **F69** every integration test with `.fetchall()` / `list_changes()` has a ≥10K-row variant. **F72** every named cross-layer integrity invariant has fixture-scale + soak-scale tests. **F81** CI fresh-install smoke (clean dir → compose boot → healthz → MCP handshake → wizard 200 → BM25 hit); per-commit leg checks the wiring. **F82** no wall-clock ceiling assertions outside soak/probe tiers (`slow`/`soak`/`load`/`pvt` marker or `# F82-allowed:` rationale required — #493 flake family). **F84** every production config-write site (`write_config_updates`/`update_config_file`/`write_config_yaml`/config-writer-named `yaml.dump`) has a composed write→read round-trip test through the canonical layered reader (#492 overlay split-brain class).
...
- **F3** every per-line suppression (`# noqa`/`# NOSONAR`/`# pragma`/`# type: ignore`/`# nosec`) has rationale. **F10** CI workflow silencers (`continue-on-error: true`) require rationale. **F14** every `sonar.issue.ignore` entry has a preceding rationale. **F16** cognitive complexity ≤ 15 per function (Sonar S3776). **F17** no string literal ≥10 chars duplicated ≥3 times (S1192). **F18** no commented-out code (S125). **F19** unused parameters must be `_`-prefixed (S1172). **F20** empty function bodies require docstring or `# Intentionally empty —` comment (S1186). **F21** every `check_*.{py,sh}` failure-output carries `fix:`/`next:`/`run:` action markers. **F23** every top-level directory has a `README.md` resolver. **F83** shell gate scripts follow the gate-runner contract — no unguarded `VAR=$(...)` under `set -e`, `|| true` needs trailing rationale, shellcheck-clean at error severity, named stage OK/FAIL verdicts in `safe-commit.sh`/`run-all.sh` (#483 class).
