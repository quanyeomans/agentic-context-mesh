...
If secrets are stored in environment files (e.g., `/opt/<service-user>/env/service.env`), restrict permissions: `chmod 600 /opt/<service-user>/env/service.env` owned by the service account only.
...
The VM service account requires:
