...
# Suffix-based exempts for build artefacts that take a project-name prefix.
# ``kairix.egg-info`` is generated by setuptools during pip install -e;
# matching by suffix avoids hard-coding the project name.
_README_EXEMPT_SUFFIXES: tuple[str, ...] = (".egg-info",)
...
def _is_exempt(name: str) -> bool:
    ...
    build artefacts that take a project-name prefix like ``*.egg-info``)."""
    if name in _README_EXEMPT:
        return True
    if any(name.endswith(suffix) for suffix in _README_EXEMPT_SUFFIXES):
        return True
