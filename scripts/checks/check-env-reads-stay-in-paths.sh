...
# kairix/secrets/** — anywhere else bypasses the canonical KairixPaths
...
# happens at the boundary in paths.py / secrets/**, never scattered.
#
# kairix/secrets/ is a package (loader.py, naming.py, _legacy.py,
# _legacy_aliases.py, cli.py) — the whole subtree is the secrets
# boundary, since the canonical credential loader composes the
# legacy chain with env-var alias resolution.
...
grep -rEl 'os\.environ.*KAIRIX_' kairix/ --include='*.py' 2>/dev/null \
    | grep -vE '^kairix/paths\.py$' \
    | grep -vE '^kairix/secrets/' \
