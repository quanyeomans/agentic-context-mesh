...
Earlier in this release line, the capability/skill recommender (`kairix recommend`) also shipped — behind the default-off `recommender` flag, so both `kairix recommend` and the `recommend_capabilities` MCP tool return a disabled reply until an operator turns it on. See the [Recently shipped](#recently-shipped) section. (The `kairix mcp-calls` per-call *analytics* layer shipped; the full agent-feedback *eval loop* it was meant to feed — EPIC #465 — was retired NOT_PLANNED and re-derived as [#633](https://github.com/three-cubes/kairix/issues/633).)
...
| Capability / skill recommender | ✅ Shipped (flag-off) | `kairix recommend` — ranks the kairix tool or local skill that fits a described task (PR #569/#570). Behind the default-off `recommender` flag: both the CLI and the `recommend_capabilities` MCP tool return a disabled reply until an operator turns it on. |
...
| Chunk expansion | ✅ Shipped v2026.7.1 | `kairix expand` / `expand` tool — pull the chunks around a search hit (the part before, the part after, the section it belongs to) within a token budget, instead of re-reading the whole document |
| Resolvable source link on every result | ✅ Shipped v2026.7.1 | Search, briefings, fact lookups, timelines, research, and claim checks all carry the same openable pointer back to the original source |
| Briefing sources footer | ✅ Shipped v2026.7.1 | Each briefing ends with a "Sources" list (at least three citations) you can open to check the work behind it |
| Incremental memory write | ✅ Shipped v2026.7.1 | Saving a memory updates just that entry instead of rescanning and rebuilding the whole index; the write always lands somewhere writable and searchable |
| Memory write-access preflight | ✅ Shipped v2026.7.1 | `kairix doctor` checks kairix can actually write where it's pointed and gives a plain-language fix when it can't (and importing a chat works on read-only deployments) |
...
**The shipped reference library (≈6,000 open-source engineering docs) runs through a standard 242-case gold-suite benchmark every release.** This is the single most useful number we publish: it shows what kairix actually delivers on a corpus you have access to and can re-run yourself.

This clean reference-library sweep is a separate upper-bound measurement — distinct from the 242-case production baseline above, not a re-measurement of it. Latest sweep (2026-05-08, post eval-pipeline standardisation, against the packaged `reflib-gold-v3.yaml` suite — suites ship inside the wheel under `kairix/data/suites/`). This sweep ran the 200-case suite as it stood on that date, before the #453 Phase B expansion grew it to today's 242 cases:
...
- **v2026.7.1 — Faster agents, answers you can follow up on.** Agents can now pull the text around a search hit with the new `kairix expand` command and matching `expand` tool — the part before, the part after, or the section it belongs to, up to a token budget — instead of re-reading the whole document. Every result (search, briefings, fact lookups, timelines, research, and claim checks) now carries the same openable pointer back to its source, briefings end with a "Sources" list you can check, and fact answers link to a real source you can open. Saving a memory is much faster (it updates just that entry instead of rebuilding the whole index) and always lands somewhere writable and searchable; `kairix doctor` now warns up front when it can't write where it's pointed, and importing a chat works on read-only deployments. Startup also warms the result-ranking model and sizes the worker pool so the first search after a restart isn't sluggish, and operator/agent-supplied file paths are kept inside their allowed area.
...
- **Capability / skill recommender** (`kairix recommend`, PR #569/#570) — give it a plain-English task and it ranks the kairix tool or local skill that fits, so agents reach for the right surface first. Ships behind the default-off `recommender` flag: `kairix recommend` and the `recommend_capabilities` MCP tool both return a disabled reply until an operator turns it on, so installing it is a no-op until you choose to enable it.
...
- **Topology operator-config surface** — declare connectors / credentials / cc_pairs / collections / scope profiles / skills in `kairix.config.yaml` instead of code. Gated by `topology_config`.
- **SharePoint, Slack, GitHub, and Notion connectors** — each shipped behind its own `connector_<name>` flag with a matching `topology_<name>` per-source-unit Container pilot (per-drive, per-channel, per-repo, per-page-tree).
...
- **Latency tail investigation** ([#436](https://github.com/three-cubes/kairix/issues/436)) — post-warm search p95 (~640ms) exceeds the 500ms benchmark threshold. Investigate the BM25 / vector / rerank dispatch hot path. The reliability story is solid; latency is the next blocker for "kairix first, every task" adoption. *Progress:* v2026.7.1 took the first bite at this from the *cold-start* side — startup now warms the result-ranking model and sizes the worker pool so the first search after a restart isn't sluggish; the steady-state post-warm p95 tail is still the open work here.
