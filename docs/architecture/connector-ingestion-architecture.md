...
| **5 (KFEAT-005 P1 connectors, flag-gated, 3 worktrees)** | KP-1 dex_crm (flag `connector_dex_crm`) · KP-2 m365_email_headers (flag `connector_m365_email_headers`) · KP-3 m365_calendar (flag `connector_m365_calendar`). Each lands at `introduce` stage default-off per `feature-flag-architecture.md`; auth via existing `kairix.secrets.get_secret()` chain — `connector-dex-api-key` for Dex; `connector-m365-{tenant-id,client-id,client-secret}` for M365 client-credentials. Both-branch tests + E2E required by F54. Operator opt-in via `kairix.config.yaml` `features:` overlay; cutover protocol per flag flip. | yes | Wave 2 + Feature-flag PRs 1-4 |
