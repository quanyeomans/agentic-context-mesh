...
| **2 (Obsidian end-to-end) — IM-1..IM-5 DONE 2026-05-23; IM-6 PENDING** | IM-1 CursorStore + DeadLetterStore impls with caller-owned-commit atomicity (`77667c06`) · IM-2 ConnectorPipeline.run_batch + FilesystemBronzeStore + DefaultSilverProcessor + registry iter_* (`d954a053`) · IM-3 worker `_default_connector_sync` + `ConnectorSyncDeps` + `_SqliteChunkWriter`/`_SqliteEntityGraphSink` (`26ebc0c5`) · IM-4 passthrough + markitdown extractors + entry-points + BDD (`286ab5bb`) · IM-5 Obsidian connector with watchdog + reconciliation (`b6c23b58`). **IM-6 (functional parity vs legacy DocumentScanner on dogfood VM + delete the legacy scanner) deferred** — requires shared-infra deployment, awaits explicit user authorisation per `feedback_release_hitl`. | yes | Wave 1 |
...
| **4 (Office mixed-media, 3 worktrees)** | OF-1 pptx (slide-aware + speaker notes) · OF-2 docx (heading-aware) · OF-3 xlsx (sheet-as-document) | yes | Wave 3 |
| **5 (KFEAT-005 P1 connectors, 3 worktrees)** | KP-1 dex_crm · KP-2 m365_email_headers · KP-3 m365_calendar | yes | Wave 2 |
| **6 (SharePoint)** | SP-1 Graph delta connector; sensitivity tier wiring exercised end-to-end | foreground | Wave 5 |
| **7 (deferred)** | Vision-enhanced extraction (KFEAT-012 Phase 3); Teams transcripts (KFEAT-005 P3-5); Curator-side EntitySignal → Neo4j push | — | Wave 6; Curator |
